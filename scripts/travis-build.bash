...
function npm-publish-timestamp () {
    ...
    local data
    printf -v data '{"state":"success","target_url":"%s","description":"Pre-release NPM module publication","context":"npm/module-bash/prerelease"}' "$url"
    if ! curl -s -H 'Accept: application/vnd.github.v3+json' \
            -H 'Content-Type: application/json' \
            -H "Authorization: token $GITHUB_TOKEN" \
            -X POST -d "$data" "https://api.github.com/repos/$TRAVIS_REPO_SLUG/statuses/$sha" > /dev/null
    then
